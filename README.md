...
- `cache-db` 持久化 DNS 缓存，参数是 db 文件路径。
  ...
  - 为降低性能开销，load/dump 没有做校验，请勿修改 db 文件，请勿跨平台共享 db 文件。
  - 有时可能需要清空 db 文件来丢弃旧缓存（关闭进程，清空文件，重新启动），例如：
...
- `verdict-cache-db` 持久化 verdict 缓存，参数是 db 文件路径。
  - 启动时，从 db 文件加载缓存数据；退出时，将缓存数据写回至 db 文件。
  - “退出”是指进程收到了`SIGTERM/SIGINT`信号，即`kill <PID>`或`CTRL+C`发送的信号。
  - 发送`SIGUSR1`信号将触发缓存写回，若未指定 db 文件，则路径为`/tmp/chinadns@verdict-cache.db`。
  - db 文件格式是“纯文本”，允许手动编辑，第一个字段为“是否大陆域名(1是0否)”，第二个字段为“域名”。
