...
pub inline fn to_ascii_namelen(qnamelen: c_int) c_int {
    return c.dns_ascii_namelen(qnamelen);
}

pub inline fn to_question_len(qnamelen: c_int) u16 {
...
pub inline fn update_ttl(records: []u8, elapsed_sec: u32) ?bool {
    var expired = false;
    return if (c.dns_update_ttl(records.ptr, cc.to_isize(records.len), elapsed_sec, &expired))
        expired
    else
        null;
}
