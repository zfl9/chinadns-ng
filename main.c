...
typedef struct queryctx {
    ...
    u16_buf_s *restrict trustdns_buf;  /* [value] {uint16_t len; char buf[];} */
...
static int s_remote_sockfds[SERVER_MAXCOUNT];

static uint16_t    s_unique_msgid = 0;
static queryctx_t *s_context_list = NULL;

static void *restrict s_packet_buf = (char [DNS_PACKET_MAXSIZE]){0};
...
static void handle_local_packet(void) {
    unlikely_if (MYHASH_CNT(s_context_list) >= 65536) { /* range:0~65535, count:65536 */
    ...
    IF_VERBOSE {
        ...
        LOGI("query [%s] from %s#%u (%u)", s_name_buf, s_ipstr_buf, port, s_unique_msgid);
    }
    ...
    if (g_noaaaa_query & (NOAAAA_TAG_GFW | NOAAAA_TAG_CHN | NOAAAA_TAG_NONE) && qtype == DNS_RECORD_TYPE_AAAA) {
        ...
        if (filter) {
            ...
            unlikely_if (sendto(s_bind_sockfd, s_packet_buf, packet_len, 0, &source_addr.sa, source_addrlen) < 0) {
                portno_t port = 0;
                parse_socket_addr(&source_addr, s_ipstr_buf, &port);
                LOGE("failed to send dns reply packet to %s#%u: (%d) %s", s_ipstr_buf, port, errno, strerror(errno));
            }
            return;
        }
    }
    ...
    for (int i = 0; i < SERVER_MAXCOUNT; ++i) {
        ...
        const skaddr_u *restrict addr = &g_remote_skaddrs[i];
        const socklen_t addrlen = skaddr_size(addr);
...
static inline bool accept_chinadns_reply(const void *restrict packet_buf, ssize_t packet_len, size_t namelen) {
...
static void handle_remote_packet(int index) {
    ...
    const char *restrict remote_ipport = g_remote_ipports[index];
    ...
    char *restrict name_buf = g_verbose ? s_name_buf : NULL;
    size_t namelen = 0;
    unlikely_if (!dns_reply_check(s_packet_buf, packet_len, name_buf, &namelen)) return;

    queryctx_t *restrict context = NULL;
    dns_header_t *dns_header = s_packet_buf;
    MYHASH_GET(s_context_list, context, &dns_header->id, sizeof(dns_header->id));
    if (!context) {
        LOGV("reply [%s] from %s (%u), result: ignore", s_name_buf, remote_ipport, dns_header->id);
        return;
    }
    ...
    if (is_chinadns_idx(index)) {
        if (context->name_tag == NAME_TAG_CHN || accept_chinadns_reply(s_packet_buf, packet_len, namelen)) {
            LOGV("reply [%s] from %s (%u), result: accept", s_name_buf, remote_ipport, dns_header->id);
            if (context->trustdns_buf)
                LOGV("reply [%s] from <previous-trustdns> (%u), result: filter", s_name_buf, dns_header->id);
        } else {
            LOGV("reply [%s] from %s (%u), result: filter", s_name_buf, remote_ipport, dns_header->id);
            if (context->trustdns_buf) {
                LOGV("reply [%s] from <previous-trustdns> (%u), result: accept", s_name_buf, dns_header->id);
        ...
        if (context->name_tag == NAME_TAG_GFW || context->chinadns_got) {
            ...
            if (context->trustdns_buf) {
                ...
                LOGV("reply [%s] from %s (%u), result: delay", s_name_buf, remote_ipport, dns_header->id);
    ...
    MYHASH_DEL(s_context_list, context);
...
static void handle_timeout_event(queryctx_t *context) {
    LOGE("upstream dns server reply timeout, unique msgid: %u", context->unique_msgid);
    MYHASH_DEL(s_context_list, context); /* delete query context from the hashtable */
...
int main(int argc, char *argv[]) {
    ...
    LOGI("local listen addr: %s#%u", g_bind_ipstr, g_bind_portno);
    ...
    if (g_repeat_times > 1) LOGI("enable repeat mode, times: %u", g_repeat_times);
    ...
    if (epoll_ctl(s_epollfd, EPOLL_CTL_ADD, s_bind_sockfd, &ev)) {
    ...
    for (int i = 0; i < SERVER_MAXCOUNT; ++i) {
        ...
        if (epoll_ctl(s_epollfd, EPOLL_CTL_ADD, s_remote_sockfds[i], &ev)) {
            LOGE("failed to register epoll event: (%d) %s", errno, strerror(errno));
            return errno;
        }
    }
